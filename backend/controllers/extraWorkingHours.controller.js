...
const ExtraWorkingHoursDocument = require('../database/models/ExtraWorkingHoursDocument');
...
class ExtraWorkingHoursController {
  async createApplication(req, res) {
    try {
      const { start_date, start_time, end_date, end_time, total_hours, reason, description, user_id, flow_type, application_date } = req.body;
      ...
      const application = await ExtraWorkingHoursApplication.create(applicationData);
      ...
      if (req.files && req.files.length > 0) {
        ...
        const targetFolder = path.join(uploadDir, 'extra-working-hours-documents', application.id.toString());
        ...
        for (const file of req.files) {
          ...
          const documentData = {
            extra_working_hours_application_id: application.id,
          ...
          const document = await ExtraWorkingHoursDocument.create(documentData);
          ...
          if (departmentGroupsForEmail && departmentGroupsForEmail.length > 0) {
            ...
            if (currentStep && currentStep.delegation_group_id) {
              ...
              if (approvers && approvers.length > 0) {
                await emailService.sendExtraWorkingHoursApprovalNotification(application, approvers, currentStage);
              }
            }
          }
        } catch (error) {
          console.error('[ExtraWorkingHoursController] 發送 email 通知失敗:', error);
        }
      }
  ...
  async getApplications(req, res) {
    try {
      ...
      const applications = await ExtraWorkingHoursApplication.findAll(options);
  ...
  async getApplicationById(req, res) {
    try {
      ...
      const application = await ExtraWorkingHoursApplication.findById(id);
  ...
  async getPendingApprovals(req, res) {
    try {
      const applications = await ExtraWorkingHoursApplication.getPendingApprovals(req.user.id);
      res.json({ applications });
    } catch (error) {
  ...
  async approve(req, res) {
    try {
      ...
      if (action === 'reject') {
        ...
        await ExtraWorkingHoursApplication.reject(id, req.user.id, remarks || '已拒絕');

        // 發送拒絕通知給申請者（Email 發送失敗不應影響流程）
        try {
          const rejectedApplication = await ExtraWorkingHoursApplication.findById(id);
          if (rejectedApplication) {
            await emailService.sendExtraWorkingHoursRejectionNotification(
              rejectedApplication,
              rejectedApplication.rejection_reason || remarks || '已拒絕'
            );
          }
        } catch (error) {
          console.error('[ExtraWorkingHoursController] 發送拒絕通知失敗:', error);
        }
      ...
      if (action === 'approve') {
        ...
        const updatedApplication = await ExtraWorkingHoursApplication.approve(id, req.user.id, currentLevel, remarks);
...
module.exports = new ExtraWorkingHoursController();
